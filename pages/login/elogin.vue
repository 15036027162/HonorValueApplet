<template>
	<view>
		<view class="content">
			...
			<view class="header-title">炎陵黄桃大数据</view>
			<view class="set-content">
				<form>
					<view class="uni-form-item uni-column form-list">
						<view class="title">手机号</view>
						<view class="setpwd">
							<input class="uni-input" type="number" :maxlength="11" v-model="form.mobile" focus placeholder="请输入手机号" />
						</view>
					</view>
					<view class="uni-form-item uni-column form-list">
						<view class="title">验证码</view>
						<view class="setpwd">
							<input class="uni-input" type="text" v-model="form.captcha" focus placeholder="请输入验证码" />
							<view class="verify" @click="sendcms()" v-if="isSend">获取验证码</view>
							<view class="verify_num"  v-else>{{num}}秒后重新获取</view>
						</view>
					</view>
					<checkbox-group class="agreement " @change="checkedChange">
						<checkbox class="checkbox round" value="agree" style="transform:scale(0.7)"></checkbox>
						<view class="text">
							我已阅读并同意
							<view class="inline" @click="ToArguement">《用户服务协议》</view>
							与
							<view class="inline" @click="ToArguement">《隐私条款》</view>
						</view>
					</checkbox-group>
					<view class="set-sub">
						<button type="primary" :disabled="!isChecked" @click="loginClick()">登录</button>
					</view>
				</form>
			</view>
			<!-- <view class="other-login">
				<view class="other-phone" @click="navigateClick('/pages/login/index')">账号密码登录</view>
				<view class="other-phone" @click="navigateClick('/pages/login/forget')">忘记密码</view>
				<view @click="navigateClick('/pages/login/register')">新用户注册</view>
			</view> -->
			<view class="login-wechat">
				<view class="title">微信登录</view>
			</view>
			<uni-icons type="weixin" size="32" color="#8a8a8a"></uni-icons>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			checkedChange(e) {
				if(e.detail.value.length&&e.detail.value[0]==='agree') {
					this.isChecked = true
				}
				else {
			...
			async loginClick() {
				if(this.form.mobile == ''){
					...
					return false
				 	...
				 	//let data = await this.$api.phoneLogin(this.form)
				 	uni.setStorage({
				 		key: "Token",
				 		// data: data.token,
						data: "1334445ff333",
				 		success(res) {
							// let isNewUser = data.isRegister
							 let isNewUser = 1
							uni.getStorage({
								key: 'launchFlag',
								success(response) {
									if(response.data==='true') {
										...
										uni.redirectTo({
											url: `/pages/index/introduce?isNewUser=${isNewUser}`
										})
									}
								},
								fail() {
									...
									uni.redirectTo({
										url: `/pages/index/introduce?isNewUser=${data.isRegister}`
									})
								}
							})
				 		}
			...
			async sendcms() {
				...
				let data = await this.$api.getPhoneCode({phone: this.form.mobile})
				if(data.captcha)
				this.form.captcha = data.captcha
			}
		}
	}
</script>
...
<style lang="less" scoped>
	...
	.other-login {
		...
		.other-phone {
			border-right: 1rpx solid #3399FF;
			padding-right: 18rpx;
			margin-right: 18rpx;
		}
	}
